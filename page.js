...
let mode = 'multi';
let mapSource = 'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}';
...
const Zone = "Zone";
const AnimalsSpecies = "AnimalsSpecies";
const AnimalSamplingDuration = "AnimalSamplingDuration";
const TeamAndDay = "TeamAndDay";
// Required
...
function getInfo(rec) {
  const result = {
    id: rec.id,
    householdId: parseValue(rec[HouseholdID]),
    zone: parseValue(rec[Zone]),
    animalsSpecies: parseValue(rec[AnimalsSpecies]),
    animalSamplingDuration: parseValue(rec[AnimalSamplingDuration]),
    teamAndDay: parseValue(rec[TeamAndDay]),
...
function updateMap(data) {
  ...
  if (!(Longitude in data[0] && Latitude in data[0] && HouseholdID in data[0])) {
    showProblem("Table does not yet have all expected columns: HouseholdID, Longitude, Latitude. You can map custom columns"+
  ...
  for (const rec of data) {
    const {id, householdId, zone, animalsSpecies, animalSamplingDuration, teamAndDay, lng, lat} = getInfo(rec);
    ...
    marker.bindPopup([
      `<strong>${householdId ?? ""} - Zone: ${zone ?? ""}</strong>`,
...
function defaultMapping(record, mappings) {
  if (!mappings) {
    return {
      ...
      [Zone]: hasCol(Zone, record) ? Zone : null,
      [AnimalsSpecies]: hasCol(AnimalsSpecies, record) ? AnimalsSpecies : null,
      [AnimalSamplingDuration]: hasCol(AnimalSamplingDuration, record) ? AnimalSamplingDuration : null,
      [TeamAndDay]: hasCol(TeamAndDay, record) ? TeamAndDay : null,
      [Latitude]: Latitude,
